...
const unversionedExpected = {
  ...
  '/api/bots':['get','post'], '/api/bots/{id}':['delete','get','patch'], '/api/bots/{id}/start':['post'], '/api/bots/{id}/stop':['post'],
...
describe('OpenAPI contract',()=>{
  ...
  it('requires idempotency keys for money and bot commands',async()=>{const api=await SwaggerParser.dereference(contractPath);
    for(const [path,method] of [['/api/v1/billing/checkout','post'],['/api/v1/bots/{id}','delete'],['/api/v1/bots/{id}/start','post'],['/api/v1/bots/{id}/stop','post'],['/api/v1/bots/{id}/emergency-stop','post'],['/api/v1/bots/{id}/restart','post'],['/api/v1/admin/payments/{id}/refund','post']]){
      const header=api.paths[path][method].parameters.find(parameter=>parameter.name==='Idempotency-Key');assert.equal(header.required,true,path);}});
