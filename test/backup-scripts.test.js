...
const scripts=['ops/postgres/backup.sh','ops/postgres/restore.sh','ops/postgres/verify-restore.sh','ops/postgres/ci-drill.sh','ops/run-encrypted-backup.sh'];
describe('backup and disaster recovery tooling',()=>{
  ...
  it('supports verified off-site upload of already encrypted archives',async()=>{const backup=await readFile(scripts[0],'utf8');assert.match(backup,/BACKUP_REMOTE/);assert.match(backup,/rclone .* copyto/);assert.match(backup,/Remote backup verification failed/);});
});
