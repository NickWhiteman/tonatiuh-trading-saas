...
type Outbox={id:string;recipient:string;template:'VERIFY_EMAIL'|'RESET_PASSWORD';encrypted_payload:string;attempts:number};
...
async function send(item:Outbox):Promise<void>{try{const payload=JSON.parse(new EncryptionService().decrypt(item.encrypted_payload)) as {token:string};
  const base=new URL(required('PUBLIC_APP_URL'));base.pathname=item.template==='VERIFY_EMAIL'?'/verify-email':'/reset-password';base.searchParams.set('token',payload.token);
  const verify=item.template==='VERIFY_EMAIL';await transport.sendMail({from:required('SMTP_FROM'),to:item.recipient,
    subject:verify?'Verify your Tonatiuh email':'Reset your Tonatiuh password',
    text:verify?`Verify your email: ${base.toString()}`:`Reset your password: ${base.toString()}`});
