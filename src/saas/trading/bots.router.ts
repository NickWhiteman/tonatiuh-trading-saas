...
import { authContext, requireActiveSubscription, requireRoles } from '../http/authorization';
...
botsRouter.post('/',requireRoles('OWNER','ADMIN','TRADER'),requireActiveSubscription,async(req,res,next)=>{try{const auth=authContext(req);
  ...
  const connection=await saasQuery('SELECT 1 FROM exchange_connections WHERE id=$1 AND organization_id=$2 AND enabled=true',[connectionId,auth.organizationId]);
  if(!connection.rowCount)throw notFound('Enabled exchange connection was not found.');const strategy=body.strategy===undefined?'VECTOR_PROFIT':stringValue(body.strategy,'strategy',40);
  if(strategy!=='VECTOR_PROFIT')throw new SaasHttpError(400,'UNSUPPORTED_STRATEGY','Strategy is not supported.');
  const result=await saasQuery(`INSERT INTO trading_bots(organization_id,exchange_connection_id,name,strategy,configuration) VALUES($1,$2,$3,$4,$5) RETURNING *`,
...
for(const command of ['START','STOP','RESTART'] as const){botsRouter.post(`/:id/${command.toLowerCase()}`,
  requireRoles('OWNER','ADMIN','TRADER'),...(command==='STOP'?[]:[requireActiveSubscription]),async(req,res,next)=>{try{
    ...
    const result=await saasTransaction(async(client)=>{const bot=(await client.query<{actual_state:string}>(
      ...
      if(inserted){const desired=command==='STOP'?'STOPPED':'RUNNING';const state=command==='STOP'?'STOPPING':'STARTING';
