...
import { setPlatformAdminDatabaseContext } from '../db/access-context';
...
export const requirePlatformAdmin:RequestHandler=async(req,_res,next)=>{try{const auth=authContext(req);const result=await saasQuery(
  ...
  if(!result.rowCount)throw new SaasHttpError(403,'PLATFORM_ADMIN_REQUIRED','Platform administrator access is required.');setPlatformAdminDatabaseContext();next();
