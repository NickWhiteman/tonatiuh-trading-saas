...
export function registerBusinessMetrics(registry:Registry):void{
  ...
  const refunds=new Gauge({name:'tonatiuh_refunds',help:'Refunds by status',labelNames:['status'],registers:[registry],async collect(){await bounded(this,'SELECT status,count(*)::int count FROM billing_refunds GROUP BY status','status');}});void refunds;
  ...
  new Gauge({name:'tonatiuh_billing_reconciliation_overdue',help:'Payment and refund reconciliations overdue by more than one hour',registers:[registry],async collect(){const row=(await saasQuery<{count:number}>(`SELECT (SELECT count(*) FROM billing_payments WHERE reconcile_after<now()-interval '1 hour')+(SELECT count(*) FROM billing_refunds WHERE reconcile_after<now()-interval '1 hour') count`)).rows[0];this.set(Number(row?.count??0));}});
}
