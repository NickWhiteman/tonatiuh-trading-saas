...
type DatabaseAccessContext = { organizationId?: string; userId?:string; platformAdmin?: boolean; service?: boolean };
...
export function setUserDatabaseContext(userId:string):void{const context=storage.getStore();if(context)context.userId=userId;}
...
export async function applyDatabaseContext(client: PoolClient): Promise<void> {
  ...
  await client.query(`SELECT set_config('app.current_organization_id',$1,true),set_config('app.current_user_id',$2,true),set_config('app.platform_admin',$3,true),
    set_config('app.service_access',$4,true),set_config('app.worker_access',$5,true)`,[
    context?.organizationId??'',context?.userId??'',context?.platformAdmin?'true':'false',context?.service||processScope==='service'?'true':'false',processScope==='worker'?'true':'false',
  ]);
}
