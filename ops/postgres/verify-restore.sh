...
[ "$migrations" -ge 15 ] || { echo "Restore verification failed: only $migrations migrations found." >&2;exit 1; }

missing_tables=$(scalar "SELECT count(*) FROM unnest(ARRAY['users','organizations','organization_memberships','exchange_connections','trading_bots','bot_commands','subscriptions','billing_payments','billing_refunds','feature_flags','consent_events','data_subject_requests','audit_events']) expected(name) WHERE to_regclass('public.'||name) IS NULL")
